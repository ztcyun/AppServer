    ...
    public class FileStorageService<T> //: IFileStorageService
    {
        ...
        public async Task<DataWrapper<T>> GetFolderItems(T parentId, int from, int count, FilterType filter, bool subjectGroup, string ssubject, string searchText, bool searchInContent, bool withSubfolders, OrderBy orderBy)
        {
            ...
            var result = new DataWrapper<T>
            {
                ...
                FolderInfo = parent,
                New = await FileMarker.GetRootFoldersIdMarkedAsNew(parentId)
            };
        ...
        public async Task<List<FileEntry>> GetNewItems(T folderId)
        {
            try
            {
                Folder<T> folder;
        ...
        public async Task<(List<object>, List<object>)> MoveOrCopyFilesCheck<T1>(IEnumerable<JsonElement> filesId, IEnumerable<JsonElement> foldersId, T1 destFolderId)
        {
            var checkedFiles = new List<object>();
            var checkedFolders = new List<object>();

            var (filesInts, folderInts) = await MoveOrCopyFilesCheck(
            ...
            var (filesStrings, folderStrings) = await MoveOrCopyFilesCheck(
